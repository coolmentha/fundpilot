...
vi.mock('../api/hooks.js', () => ({
    useFunds: () => ({data: [{id: 1, portfolioFundId: 11, fundName: '目标基金'},
        {id: 2, portfolioFundId: 12, fundName: '其他基金'}]}),
    usePendingTransactions: () => ({
        data: [{id: 11, fundId: 1, portfolioFundId: 11, status: 'PENDING'},
            {id: 12, fundId: 2, portfolioFundId: 12, status: 'PENDING'}],
...
describe('ConfirmPage', () => {
    ...
    it('按 portfolioFundId 查询参数筛选待确认交易', async () => {
        container = document.createElement('div');
        document.body.appendChild(container);
        root = createRoot(container);
        await act(async () => root.render(
            <MemoryRouter initialEntries={['/confirm?portfolioFundId=11']}><App><ConfirmPage/></App></MemoryRouter>,
        ));

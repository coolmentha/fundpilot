...
class IdentityAccessExceptionHandler {
    ...
    ResponseEntity<ApiResponse<Void>> authentication(AuthenticationFailure failure) {
        ...
        return response.body(ApiResponse.error(failure.code().name(), failure.getMessage()));
    }

    @ExceptionHandler(UserAdministrationFailure.class)
    ResponseEntity<ApiResponse<Void>> administration(UserAdministrationFailure failure) {

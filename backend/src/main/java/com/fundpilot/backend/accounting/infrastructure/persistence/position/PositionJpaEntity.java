...
/** 持仓投影实体；Accounting 自 V40 起独占 {@code opened_at}/{@code cost_per_share}/{@code status}。 */
@Entity
@Table(name = "accounting_position")
@EntityListeners(AuditingEntityListener.class)
@SQLRestriction("deleted_date IS NULL")
@SQLDelete(sql = "UPDATE accounting_position SET deleted_date = now() WHERE id = ? AND version = ?")
@Getter
@Setter

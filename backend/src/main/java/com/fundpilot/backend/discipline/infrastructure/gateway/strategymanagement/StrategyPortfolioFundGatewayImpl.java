...
@Component @RequiredArgsConstructor public class StrategyPortfolioFundGatewayImpl implements StrategyPortfolioFundGateway {
    ...
    @Override public PortfolioFund requireTrackedForUpdate(long ownerId, long portfolioFundId) { return checked(funds.findForUpdate(portfolioFundId).filter(fund -> fund.ownerId() == ownerId).orElseThrow(() -> new BusinessException(ErrorCode.ENTITY_NOT_FOUND, "组合基金不存在"))); }

...
class InvestmentPlanExecutionCommandHandlerTest {
    ...
    void 交易日按计划创建带业务日的账目() {
        ...
        boolean created = new InvestmentPlanExecutionCommandHandler(plans, calendar, transactions, portfolioFunds)
                .execute(7L, MONDAY);
    ...
    void 已存在同日账目视为幂等跳过() {
        ...
        assertThat(new InvestmentPlanExecutionCommandHandler(plans, calendar, transactions, portfolioFunds)
                .execute(7L, MONDAY))
                .isFalse();
    }

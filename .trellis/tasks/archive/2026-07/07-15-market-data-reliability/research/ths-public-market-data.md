...
响应约 7 MB：`g({"data":{"info":{"count":23150,...},"data":{"f000001":{...}}}})`。 条目包含 `code`、`name`、`typename`
，可映射为现有 `FundDictEntry(code,name,rawName)`。该接口只在东方财富字典失败时调用，避免正常路径下载两份全量字典。
...
`last.js` 对主流指数返回 140 根，足够现有 VolumeState 与图表降级展示。个别主题指数返回 1 根或 total=0，应按 empty
继续/失败处理，不伪造完整历史。年文件 `/{year}.js` 可返回完整年度日线，但一次降级需要多次请求，会破坏 15 秒在线预算，本次不采用。

## Implementation Consequences

- 使用三个 raw client：fund NAV、fund info dictionary、index line；由一个 `ThsMarketDataSource` 聚合为现有
  `MarketDataSource`。
- 普通净值回退需要两个 HTTP 请求；外呼超时应收紧到 connect 1 秒/read 3 秒，并限制东方财富令牌等待，保证最坏链路仍小于前端
  15 秒。

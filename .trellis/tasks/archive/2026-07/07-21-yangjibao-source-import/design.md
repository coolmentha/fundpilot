...
本功能只负责把养基宝账户持仓转换为 FundPilot 已有业务动作，不建立长期同步关系。后端新增养基宝 API
客户端、短期导入会话与导入编排；前端在设置页提供扫码、预览、选择和结果展示。无数据库迁移、无新依赖、无后台任务，不修改行情数据源链。
...
- 新 code：调用现有 `FundService.create`，传 `initialHoldingShares=hold_share`、`costPerShare=hold_cost`
  ，类型字段缺省时沿用现有名称分类兜底；该调用自身保证基金、净值和初始 `INCREASE` 交易原子性。
- 已有 code + “以本系统为准”：返回跳过结果，不写数据库。
- 已有 code + “同步为养基宝份额”：锁后重新读取当前 CONFIRMED 事实份额，以两位份额精度计算差额。正差调用现有手动 `ADJUST_IN`
  ，负差调用 `ADJUST_OUT`，零差跳过。

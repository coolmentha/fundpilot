...
- 新增 `@Bean SinaTradingCalendarClient sinaTradingCalendarClient(@Value("${sina.base-url:https://finance.sina.com.cn}") String baseUrl)`。
...
- `sync()`：`raw = sinaClient.fetchTradingCalendarRaw()` -> `dates = SinaTradingCalendarParser.parse(raw)` -> 对表中新日期 INSERT。
...
- 后端：`cd backend && ./mvnw test`（聚焦：`-Dtest=TradingCalendarServiceTest,SinaTradingCalendarParserTest,TradingCalendarSchemaIntegrationTest`）

...
若 22:59 前仍未公布或晚间请求失败，次日 00:00-09:59 每 10 分钟按交易日历补拉上一交易日；指定日期已落库则跳过。服务必须接受显式目标交易日，不能在跨夜补拉时仍固定与“今天”比较。
_Avoid_: 用 fundgz 的 dwjz（单位净值）落库（分红基金累计净值失真）；已确认基金重复拉取（浪费请求）；只延长 cron 却仍按当天日期校验（次日上午永远无法补昨天净值）
...
**总盈亏（Total PnL）**:
基金整体赚了还是亏了。盘后 = 持仓份额 ×（最近单位净值 - 成本单价）；盘中估算 = 持仓份额 ×（最新已公布单位净值 × (1 + 今日涨跌幅) - 成本单价）。用于"盈利基金/亏损基金"分组。无持仓为 null；当日净值未入库且估值失败/缺失时，当前持仓市值与总盈亏同样为未知，不得拿上一期已公布净值冒充当前值。

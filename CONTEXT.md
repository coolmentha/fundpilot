...
口径 = 策略收益 / 策略最大回撤（类 Calmar），既是寻优 train 集的择优标尺，也是 test 集 passed 判定的回撤维度——策略 Calmar 须 ≥ dca Calmar（接受超额回撤，但单位回撤换的收益不劣于定投）。train 侧在候选间排序选最高，test 侧与 dca 比较定通过；同源口径，择优标尺与验证标尺闭环。规避过拟合到激进参数。
_Avoid_: 收益回撤比（偏实现术语）；纯收益率最大化（可能选出回撤极大的激进参数）；绝对回撤约束（策略回撤 ≤ dca 回撤，惩罚用合理风险换合理收益的策略，见 ADR-0010）

**样本外验证（Out-of-Sample Validation）**:
寻优防过拟合的机制。1 年回测窗口内按时间 0.7:0.3 切分——前 70%（train 集）网格搜索按风险调整收益选 top-k 候选（ADR-0011），后 30%（test 集）在 passed 候选中按 test Calmar 择优落库。无 passed 候选才算寻优失败，不落库报错。test 集从"布尔门槛"升级为"连续择优标尺"——train 单点冠军可能是噪声产物，top-k 给 test 集多个候选，降低选到过拟合参数的概率。
...
`singlePositionLimit`（单只 30%、无关类型）→ 触发。卖出金额 = `(当前占比 - 上限) × 总权益持仓金额`，按最近净值反算为份额。
...
**总盈亏（Total PnL）**:
基金整体赚了还是亏了。盘后 = 持仓份额 × 最近累计净值 - 持仓成本（当前市值 - 成本）；盘中估算 = 昨日总盈亏 × (1 + 今日涨跌幅)（口径与今日盈亏同源，都用涨跌幅比例推算）。持仓成本 = Σ CONFIRMED 买入交易 amount（INCREASE/TRANSFER_IN/INVEST 的 amount 之和，减去 DECREASE/TRANSFER_OUT 的 amount 之和）。用于"盈利基金/亏损基金"分组。无持仓为 null。
...
新建基金时录入已有持仓的建仓动作。`FundCreateRequest.existingAmount` 有值即触发——状态流转对齐 BUILD 信号确认
（`FundStatus → HOLDING`、写 INCREASE 交易），但确认时机同步：用最近一期已公布净值反算 shares、
置 CONFIRMED，不等 NavConfirmJob。金额是**当前市值口径**（"现在有多少钱"），故净值用最近一期反算；
`openedAt` 用户可填（大致建仓时点，影响移动止盈持仓期高点起算），不填用 now，须 ≤ 今天。与手动交易的本质区别：
手动交易是已建仓后的资金动作（走 NavConfirmJob 异步确认），初始持仓录入是建仓本身（同步确认）。
交易来源用 INCREASE（对齐 handleBuild 建仓语义，不用 TRANSFER_IN——建仓是首笔买入非转入）。

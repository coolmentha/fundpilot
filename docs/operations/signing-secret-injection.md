...
生产沿用仓库根目录的受控 `.env` 或宿主环境，由 `deploy/docker-compose.prod.yml` 将 `YANGJIBAO_SECRET` 传入后端容器；自动部署重写其余环境项时会保留该值。只填写第三方协议认可的秘密，不应任意生成替代值。示例文件保留空值；禁止将真实值提交、写入镜像或输出到日志。部署脚本与 Compose 对缺失或空值提前报错，应用另行拒绝全空白值。

> **历史资料**：本文是组合盈亏与手动交易功能的早期 PRD，正文包含后来被 ADR-0015、ADR-0019 和 ADR-0020 取代的计划仓位及净值口径。当前规则请查看外部工作台提供的领域上下文和[业务逻辑文档](business/README.md)。
...
- 手动卖出不经过 `evaluateSignal`，不卡 7 天硬约束（工作台领域上下文已明确，前端可提示但不阻止）。
...
- **定投计划**：本期手动定投是"每期录一条单次交易"，不建定投计划（DcaPlanEntity）、不做自动扣款。工作台领域上下文明确"不做定投的话另起 DcaPlanEntity 表"。
- **基金转换的互指关系**：手动转入/转出作为独立交易录入，不在本期自动建立 `relatedTransaction` 互指（用户分两笔录）。自动互指留待将来。
- **历史范围说明（已被 ADR-0008 取代）**：本 PRD 当时不做盘中估值；现行契约已改为 fundgz 三态今日涨跌，T-1 vs T-2 是昨日涨跌，不得作为今日值。
- **Redis 缓存**：盈亏计算实时聚合，不引入缓存（工作台领域上下文已明确本期不引入 Redis）。
- **多用户**：本期单用户，不预留 userId。

## Further Notes

- **两个正交维度**：上涨/下跌（今日净值变动率）与盈利/亏损（总盈亏正负）是两个维度，一只基金可能今日上涨但整体亏损。前端展示时两者独立计数，不混用。
- **今日盈亏 vs 总盈亏**：现行今日盈亏随三态变化，盘中/待公布用昨日市值 × fundgz 今日估算涨跌幅，盘后用当日累计净值切换实际值；总盈亏反映当前市值相对持仓成本的累计盈亏。两者口径不同，概览页同时展示。
- **计划仓位校验与硬约束互补**：计划仓位校验管"意图上限"（建仓时防死状态），硬约束管"事实上限"（信号生成时卡实际持仓占比）。两者不替代，信号生成时的硬约束检查保持不变。
- 领域术语已固化在工作台领域上下文：`计划仓位校验`、`今日涨跌/今日盈亏/总盈亏`、`上涨下跌vs盈亏基金`、`手动交易`。

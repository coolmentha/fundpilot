...
- **A. 存储成本单价 + 加权维护（已采纳）**：`FundEntity.costPerShare` 单一字段。建仓时直接录入 `initialHoldingShares`，costPerShare 用户可填(不填用 T-1净值)。后续 INCREASE/TRANSFER_IN/INVEST 交易 CONFIRMED 时同一事务内加权：`新 = (旧×旧份额 + amount) / (旧+新)`。卖出不触发重算。总盈亏 = `shares × (latestNav - costPerShare)`。

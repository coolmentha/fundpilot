...
> **Status: partially superseded by [v0.14.0 版本计划](../versions/v0.14.0-plan.md)** —— 「定投是自动执行机制、非信号」的决策仍有效；
> 但「止盈由基金绑定的 `FundStrategyEntity` 信号独立触发」的载体已不存在：v0.14.0 删除 discipline 模块（`fund_strategy` 亦为遗留表），
> 止盈改由 alerting 的建议型提醒（`AlertRuleKind.TRAILING_STOP`）触发，定投与止盈两者依旧解耦。

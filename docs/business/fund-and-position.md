...
产品目录 `FundProduct` 维护基金身份与行情分类，`PortfolioFund` 维护用户是否跟踪该产品及组合配置，Accounting `Position` 维护事实持仓。事实持仓不存冗余金额，由 CONFIRMED 交易份额实时聚合。

本流程不负责决定买哪只基金，也不自动建仓。用户可以创建空仓观察基金、录入已有持仓，或后续通过手动交易和自动定投增加份额。
...
| 字段 | 用途 |
| --- | --- |
| `DefaultDisciplineCategory` | 宽基、行业、主动、混合；收益构成的分类来源（产品默认纪律分类，v0.14.0 起不再用于纪律参数推荐） |
| `ProductType` | ETF、指数、指数增强、主动；用于选择行情路径与逻辑破坏止损路径 |
| `FundProduct.benchmarkIndexCode` | 指数类基金的跟踪指数；主动基金可以为空 |

产品目录同步时按名称启发式补充产品类型与默认纪律分类；用户将产品加入组合时只提交 `fundProductId`，不复制一份可独立修改的产品身份。
...
stateDiagram-v2
    [*] --> EMPTY: 新建空仓基金
    EMPTY --> OPEN: 首笔买入或调增确认后净份额大于 0
    OPEN --> CLEARED: 卖出或调减后净份额不大于 0
    CLEARED --> OPEN: 后续买入或调增后净份额重新大于 0
```

状态目标由 CONFIRMED 交易决定：

| 条件 | Accounting `PositionStatus` |
| --- | --- |
| 没有 CONFIRMED 交易 | `EMPTY` |
| CONFIRMED 净份额大于 0 | `OPEN` |
...
2. 选中候选后保留其 `fundProductId`，代码、名称、类型和跟踪指数只用于确认展示。
3. 后端校验产品存在、尚未加入当前组合及仓位提醒线；仓位提醒只影响展示。
4. 保存 `TRACKED` 的组合基金；没有 CONFIRMED 账目时持仓状态为 `EMPTY`。
...
- `openedAt`：首次进入或重新进入 `OPEN` 时，从已确认正向交易的业务时间确定。
- `costPerShare`：买入确认时按实际投入金额加权；卖出不修改。
- 当前持仓成本录错时，用户可以在基金编辑入口修正 `costPerShare`；系统写入一条不改变份额的成本重置流水，后续买入从该基准继续加权。历史交易和 FIFO lot 不改写，因此历史及未来已实现盈亏仍按真实 lot 成本核算。
...
- 分组仅用于基金管理页和首页持仓页筛选，不参与持仓、收益、交易或提醒计算。
...
- 被作废组合基金的交易、收益、提醒、计划和行情关联完全排除在业务计算之外。
...
- 清仓不改变记录有效性，后续买入可重新进入 `OPEN` 并开始新的持仓周期。
...
| 用户再次买入已清仓基金 | 重新进入 `OPEN`，建立新的持仓周期，历史交易不丢失 |
...
| 产品已加入当前组合 | `PORTFOLIO_FUND_ALREADY_TRACKED` |
| 持有份额非正 | `INITIAL_HOLDING_SHARES_INVALID` |
| 成本单价非正 | `COST_PER_SHARE_INVALID` |
| 空仓或已清仓基金修改成本单价 | `PORTFOLIO_FUND_NOT_OPEN` |
| 建仓时间在未来 | `OPENED_AT_IN_FUTURE` |
| 无有效单位净值 | `NAV_UNAVAILABLE` |
...
- 测试：[PortfolioFundTest](../../backend/src/test/java/com/fundpilot/backend/portfolio/domain/portfoliofund/PortfolioFundTest.java)、[PortfolioFundOnboardingIntegrationTest](../../backend/src/test/java/com/fundpilot/backend/accounting/adapter/web/fundonboarding/PortfolioFundOnboardingIntegrationTest.java)、[PortfolioCorrectionIntegrationTest](../../backend/src/test/java/com/fundpilot/backend/accounting/adapter/web/portfoliocorrection/PortfolioCorrectionIntegrationTest.java)、[FundGroupingIntegrationTest](../../backend/src/test/java/com/fundpilot/backend/portfolio/adapter/web/fundgrouping/FundGroupingIntegrationTest.java)

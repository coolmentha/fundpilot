...
行情模块为工作台展示、提醒评估和交易确认提供不同时间尺度的数据。实时估值、日内行情、已确认净值和指标计算数据不能混为同一份数据。
...
| `market_indicator_snapshot` | PostgreSQL | 14:30、14:40、14:50 三批 | 基金详情行情指标展示 |
| `fund_nav_history` | PostgreSQL | 日间拉历史；当晚确认当日净值 | 交易确认、盈亏、年线、MACD、回撤 |
| `index_kline` | PostgreSQL | 行情指标拉取时增量写入；交易日 17:00–22:00 每整点补拉收盘 K 线 | K 线图和量能指标 |
...
- 非 QDII 的 `TRACKED` 组合基金都进入估值刷新，`PositionStatus.EMPTY` 的观察项也能看到今日涨跌。
...
单只基金失败时跳过该基金当日快照并继续其他基金。

提醒评估（[AlertEvaluationJob](../../backend/src/main/java/com/fundpilot/backend/alerting/adapter/scheduler/ruleevaluation/AlertEvaluationJob.java)，14:30）是独立任务，指标按需计算，**不依赖这三批快照**，快照批次失败也不影响提醒评估。
...
收益率口径：`持仓收益率 = 总盈亏 / (当前持仓市值 - 总盈亏)`，分母即当前持仓成本。持仓市值缺失或成本不为正时为 null，页面只显示金额。基金列表和基金详情必须使用同一口径，不得各算一套。
...
交易日北京时间 17:00、18:00、19:00、20:00、21:00、22:00 补拉跟踪基金的去重基准指数。
目标交易日的 OHLC 为正、满足高低价关系、成交量非负且在当日 17:00 后更新入库，视为完整并跳过；盘中缓存仍需补拉一次。
数据源未返回目标日完整 K 线或单个指数失败时，不重复写旧数据，留待下一轮；已完整状态通过数据库判定，重启后仍可跳过。
外部源不提供终版标志，此处完整表示收盘后取得的有效日 K，不保证后续发布方不会修订。写入沿用指数与交易日期唯一键 upsert。
补拉只更新 K 线，不发布行情指标完成事件。手动入口为基金详情 → 行情指标 →「刷新行情」，会主动刷新该基金行情（含 K 线）。
...
- 实现：[MarketRealtimeCache](../../backend/src/main/java/com/fundpilot/backend/marketdata/infrastructure/cache/realtimevaluation/MarketRealtimeCache.java)、[FundEstimateService](../../backend/src/main/java/com/fundpilot/backend/marketdata/infrastructure/cache/realtimevaluation/FundEstimateService.java)、[NavPublishingCommandHandler](../../backend/src/main/java/com/fundpilot/backend/marketdata/application/command/navpublishing/NavPublishingCommandHandler.java)、[PortfolioReturnQueryHandler](../../backend/src/main/java/com/fundpilot/backend/insights/application/query/portfolioreturn/PortfolioReturnQueryHandler.java)、[IndexKlineCommandHandler](../../backend/src/main/java/com/fundpilot/backend/marketdata/application/command/indexkline/IndexKlineCommandHandler.java)
- 测试：[MarketRealtimeCacheTest](../../backend/src/test/java/com/fundpilot/backend/marketdata/infrastructure/cache/realtimevaluation/MarketRealtimeCacheTest.java)、[FundEstimateServiceTest](../../backend/src/test/java/com/fundpilot/backend/marketdata/infrastructure/cache/realtimevaluation/FundEstimateServiceTest.java)、[NavPublishingCommandHandlerTest](../../backend/src/test/java/com/fundpilot/backend/marketdata/application/command/navpublishing/NavPublishingCommandHandlerTest.java)、[PortfolioReturnQueryHandlerTest](../../backend/src/test/java/com/fundpilot/backend/insights/application/query/portfolioreturn/PortfolioReturnQueryHandlerTest.java)、[PortfolioInsightsEntryIntegrationTest](../../backend/src/test/java/com/fundpilot/backend/insights/adapter/web/portfolioreturn/PortfolioInsightsEntryIntegrationTest.java)

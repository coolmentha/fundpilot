...
| 数据 | 存储 | 刷新/落库 | 主要消费者 |
| --- | --- | --- | --- |
| 指数、市场宽度、板块、基金估值与当日分时 | 后端内存副本 + Redis AOF | 交易时段每 30 秒；启动先恢复 Redis，再异步刷新 | 行情工作台、三态涨跌、基金详情 |
...
- 盘中估值优先同花顺分钟线，失败时降级东方财富 fundgz 单点估值；同一轮只拉取一次相同基准指数 K 线。
...
- A 股交易时段刷新全部平台基金估值，扩展时段只刷新 QDII；每轮估值固定运行 25 秒，未完成的基金和批量分页在下轮从断点继续。

...
这是 ADR-0021 确认的有意产品变更：仓位提醒替代旧版单基金硬上限。系统负责暴露集中度风险，由用户决定是否继续交易，不再用资金配置阻断真实交易和持仓录入。

## 月度定投预算

- 用户配置可空的 `monthlyDcaBudget`；保存直接覆盖旧值，清空后不再比较预算。
- 摘要按北京时间自然月计算。已定投为所有未取消的 `INVEST`，包括手动/自动和 `PENDING`/`CONFIRMED`。
- 本月剩余预计为当前 `EFFECTIVE` 且 `enabled=true` 的计划在本月尚未生成交易的实际执行日金额。已为同一计划创建任意状态交易的日期不得重复预测。
- 日计划、周计划和月计划复用 DCA 执行日规则；当天仅在 14:55 前算未来，月计划连续休市后按实际跨月执行日期归属。
- 未设置预算时仍显示已定投、本月剩余预计和全月预计；设置后才显示分段进度、剩余或超额。
...
- 实现：[InvestmentPlanBudgetSummaryQueryHandler](../../backend/src/main/java/com/fundpilot/backend/investmentplan/application/query/budgetmanagement/InvestmentPlanBudgetSummaryQueryHandler.java)、[InvestmentPlanExecutionCommandHandler](../../backend/src/main/java/com/fundpilot/backend/investmentplan/application/command/planexecution/InvestmentPlanExecutionCommandHandler.java)、[PortfolioFundConfigurationController](../../backend/src/main/java/com/fundpilot/backend/portfolio/adapter/web/fundtracking/PortfolioFundConfigurationController.java)（仓位提醒线由组合基金配置入口维护）
- 测试：[InvestmentPlanBudgetSummaryQueryHandlerTest](../../backend/src/test/java/com/fundpilot/backend/investmentplan/application/query/budgetmanagement/InvestmentPlanBudgetSummaryQueryHandlerTest.java)、[InvestmentPlanExecutionCommandHandlerTest](../../backend/src/test/java/com/fundpilot/backend/investmentplan/application/command/planexecution/InvestmentPlanExecutionCommandHandlerTest.java)、[PortfolioFundConfigurationIntegrationTest](../../backend/src/test/java/com/fundpilot/backend/portfolio/adapter/web/fundtracking/PortfolioFundConfigurationIntegrationTest.java)

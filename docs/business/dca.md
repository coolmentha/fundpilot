...
定投计划表示用户已经授权系统按固定周期创建买入交易。它是自动执行机制，不是提醒，也不经过提醒规则评估。
...
逐计划拆分（未来逐项预测）在基金列表和定投管理页都默认收起，折叠头显示计划条数，展开后按执行日列出每个计划的预计金额和最大金额。
...
- 定投买入与回撤止盈提醒相互独立。
- 新定投 lot 未满 5 个交易日时，只保护该 lot 不进入止盈建议份额（成熟可赎回份额）；不会冻结更早的成熟份额。
- 定投交易确认后会增加持仓、建立 lot 并更新成本单价，但不会直接触发提醒。

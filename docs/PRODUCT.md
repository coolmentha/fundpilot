...
实时接口读取后端内存副本，刷新结果写穿 Redis 并在应用重启时恢复；前端轮询不得直接击穿外部行情源。指数 K 线优先读取本地 `index_kline` 缓存，缓存空时才允许实时降级获取。
...
- `costPerShare`：用户当前持仓成本单价，不填时使用最近一期单位净值；持仓期间可修正当前基准，但不改历史交易或 FIFO lot 成本。
